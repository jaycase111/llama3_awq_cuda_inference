...
#include <re2/re2.h>
#include <optional>
#include "tokenizer-main/src/sw/tokenizer/base64.h"
#include "tokenizer-main/src/sw/tokenizer/errors.h"
#include "tokenizer-main/src/sw/tokenizer/toml.h"
...
private:
    ...
    std::unordered_map<std::string, uint64_t> encoder_;
    Config config;

public:

    explicit TiktokenFactory(const std::string model,
                             int flag
                             ){
        const std::string pattern = "(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\\r\\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}{1,3}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+";
        auto encoder = _load_encoder(model);
        int length = encoder.size();

        int num_reserved_special_tokens = 256;

        std::vector<std::string> special_tokens_vector = {
                "<|begin_of_text|>",
                "<|end_of_text|>",
        ...
        for (int i = 5; i < num_reserved_special_tokens - 5; ++i) {
            special_tokens_vector.push_back("<|reserved_special_token_" + std::to_string(i) + "|>");
        }

        std::unordered_map<std::string, uint64_t> specical_tokens;
        for (int i=0; i < special_tokens_vector.size(); i++){
            specical_tokens.insert(std::make_pair(special_tokens_vector[i], length + i));
        };
        config.special_tokens = specical_tokens;
    ...
    Tiktoken create_using_model(const std::string &model) const {
        return Tiktoken(std::move(encoder_), config.special_tokens, config.pattern);
    }
